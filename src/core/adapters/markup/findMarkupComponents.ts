...
const COMPONENT_PATH = /(?:^|\/)(?:components?|partials?|includes?|fragments?|ui)(?:\/|$)/i
...
export function findMarkupComponents(rootPath: string, candidateFiles: string[], pageAbsolutePaths: Set<string>): DetectedComponent[] {
  return candidateFiles
    .filter((filePath) => !pageAbsolutePaths.has(filePath) && COMPONENT_EXTENSIONS.test(filePath) && COMPONENT_PATH.test(filePath.split(path.sep).join('/')))
    .slice(0, 3000)
    .map((filePath) => ({
