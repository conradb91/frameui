...
/** Scan each block once. Retrying a greedy selector at every character of an
 * embedded font URL made the previous expression quadratic on real projects. */
export function extractVisibilityRules(css: string): string[] {
  const rules: string[] = []
  let segmentStart = 0
  let ruleStart = 0
  let bodyStart = 0
  for (const delimiter of css.matchAll(/[{}]/g)) {
    const offset = delimiter.index!
    if (delimiter[0] === '{') {
      ruleStart = segmentStart
      bodyStart = offset + 1
    } else {
      if (bodyStart > ruleStart && /(?:^|;)\s*display\s*:\s*none\b/i.test(css.slice(bodyStart, offset))) rules.push(css.slice(ruleStart, offset + 1))
      bodyStart = 0
    }
...
export function readProjectVisuals(root: string, files: string[], assetRoots: string[] = ['public']): ProjectVisuals {
  ...
  // Adapter-provided URL roots let all canvases resolve /logo.svg without
  // knowing whether its source lives in public, wwwroot or another directory.
  for (const [file, data] of Object.entries(assets)) {
    for (const directory of assetRoots) {
      const prefix = directory.replace(/\/$/, '') + '/'
      if (!file.startsWith(prefix)) continue
      const urlPath = file.slice(prefix.length)
      assets[urlPath] ??= data
      assets[`~/${urlPath}`] ??= data
    }
  }
  ...
  for (const file of files.filter((file) => /\.css$/i.test(file))) {
    try {
      ...
      const content = fs.readFileSync(absolute, 'utf8').replace(/@import\s+[^;]+;/gi, '').replace(/url\(\s*(['"]?)(.*?)\1\s*\)/gi, (_all, _quote, url: string) => {
        ...
        const asset = clean.startsWith('~/') ? clean.slice(2) : clean.startsWith('/') ? clean.slice(1) : path.posix.normalize(path.posix.join(path.posix.dirname(file), clean))
  ...
  return { css, assets,
    ...
    visibilityRules: extractVisibilityRules(css),
  }
}
