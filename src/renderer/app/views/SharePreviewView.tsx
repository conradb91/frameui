...
export function SharePreviewView() {
  ...
  if (!activeProject || !sharePreviewId) {
    return (
      <div className="flex h-full w-full items-center justify-center bg-[#0a0a0c] text-text-3">
  ...
  if (bundle === null) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center gap-4 bg-[#0a0a0c] px-6 text-center">
        <div className="text-[15px] font-semibold text-text">{previewMeta?.name ?? 'Share Preview'} — Not yet packaged</div>
        ...
        {previewMeta && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-full bg-gradient-to-b from-[#8676F4] to-[#7461EE] px-4 py-2 text-[12.5px] font-semibold text-white disabled:opacity-50"
          >
            {repackaging && <Loader2 size={13} className="animate-spin" />}
            Package Now
          </button>
        )}
        <button type="button" onClick={handleExit} className="text-[11.5px] font-semibold text-text-3 hover:text-text-2">
          ← Back
        </button>
      </div>
    )
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden bg-[#0a0a0c] text-text">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-border/60 px-6 py-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2 text-[10.5px] font-semibold uppercase tracking-wide text-accent-2">
            <span className="h-1.5 w-1.5 rounded-full bg-accent-2" />
            Interactive Prototype
          </div>
          <div className="truncate text-[17px] font-bold text-text">{bundle.journey?.name ?? bundle.name}</div>
        </div>

        <div className="flex items-center gap-4">
          <div className="flex items-center gap-0.5 rounded-full border border-border bg-panel-2/80 p-0.5">
            {bundle.viewports.map((vp) => {
              ...
              return (
                <button
                  ...
                  title={vp}
                  className={`flex items-center gap-1 rounded-full px-2.5 py-1.5 text-[11px] font-semibold capitalize ${
                    viewport === vp ? 'bg-accent-2/20 text-accent-2' : 'text-text-2'
                  }`}
                >
                  <Icon size={12} /> {vp}
                </button>
          ...
          <div className="flex items-center gap-1 rounded-full border border-border bg-panel-2/60 p-0.5">
            {(['overview', 'prototype', 'changes'] as Tab[]).map((t) => (
              <button
                key={t}
                type="button"
                onClick={() => setTab(t)}
                className={`rounded-full px-3 py-1.5 text-[11.5px] font-semibold capitalize ${tab === t ? 'bg-white/10 text-text' : 'text-text-2'}`}
              >
                {t}
              </button>
            ))}
          ...
          {tab !== 'prototype' && (
            <button
              ...
              className="rounded-full bg-gradient-to-b from-[#8676F4] to-[#7461EE] px-4 py-2 text-[12px] font-semibold text-white"
            >
              Start Prototype →
            </button>
          )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        {tab === 'prototype' &&
          (bundle.steps.length === 0 ? (
            <div className="flex h-full items-center justify-center px-6 text-center text-[12px] text-text-3">
            ...
            <div className="flex h-full flex-col items-center justify-center gap-6 p-10">
              <div className={`flex gap-6 ${showCompare && currentPage ? '' : 'justify-center'}`}>
                <div className="flex flex-col items-center gap-2">
                  {showCompare && currentPage && <div className="text-[10px] font-semibold uppercase tracking-wide text-text-3">Proposed</div>}
                  <div
                    className="max-h-[60vh] overflow-y-auto rounded-xl border border-border bg-panel p-8 shadow-2xl transition-[width] duration-200"
                ...
                {showCompare && currentPage && (
                  <div className="flex flex-col items-center gap-2">
                    <div className="text-[10px] font-semibold uppercase tracking-wide text-text-3">Current</div>
                    <div className="h-[60vh] w-[280px] overflow-hidden rounded-xl border border-border shadow-2xl">
              ...
              {bundle.includeCurrentComparison && currentPage && (
                <button type="button" onClick={() => setShowCompare((v) => !v)} className="text-[11px] font-semibold text-accent-2">
                  {showCompare ? 'Hide' : 'Show'} comparison against Current
                </button>
              )}

              {/* Floating navigation */}
              <div className="flex items-center gap-3 rounded-full border border-border bg-panel-2/90 py-2 pl-2 pr-3">
                <button
                  type="button"
                  disabled={history.length === 0}
                  onClick={goBack}
                  className="flex items-center gap-1 rounded-full px-3 py-1.5 text-[12px] font-semibold text-text-2 disabled:opacity-30"
                >
                ...
                {bundle.journey ? (
                  outgoingConnections.length === 0 ? (
                    <span className="px-3 py-1.5 text-[11.5px] text-text-3">End of prototype</span>
                  ) : (
                    outgoingConnections.map((conn) => (
                      <button
                        ...
                        className="whitespace-nowrap rounded-full bg-gradient-to-b from-[#8676F4] to-[#7461EE] px-3.5 py-1.5 text-[12px] font-semibold text-white"
                      >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 rounded-full bg-gradient-to-b from-[#8676F4] to-[#7461EE] px-3.5 py-1.5 text-[12px] font-semibold text-white disabled:opacity-30"
                  >
  ...
  return (
    <div className="mx-auto max-w-[520px] p-8">
      ...
      <div className="overflow-hidden rounded-lg border border-border">
        {rows.map((row, i) => (
          <div key={row.label} className={`flex items-center justify-between px-4 py-2.5 text-[12px] ${i % 2 === 0 ? 'bg-panel-2/60' : ''}`}>
...
function ChangesTab({ bundle }: { bundle: SharePackageBundle }) {
  ...
  return (
    <div className="mx-auto max-w-[480px] p-8">
      ...
      <div className="mb-4 text-[11px] text-text-3">Element-level provenance across every step included in this package.</div>
      <div className="flex flex-col gap-2">
        {rows.map(
          (row) =>
            counts[row.key] > 0 && (
              <div key={row.key} className="flex items-center justify-between rounded-md border border-border bg-panel-2/60 px-3 py-2">
