...
export function FeatureExportPanelView() {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-bg">
    <header className="flex h-[52px] shrink-0 items-center justify-between border-b border-border bg-bg-raised px-5"><div><span className="text-[13.5px] font-semibold text-text">Export Feature — {feature.name}</span><span className="ml-2 rounded bg-success/10 px-2 py-0.5 text-[9px] font-bold uppercase text-success">SVG V1 available</span></div><button type="button" onClick={() => setView('feature-workspace')} className="text-[12px] text-text-2">Close</button></header>
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(440px,1fr)_320px] gap-5 overflow-hidden p-6">
      ...
      <aside className="overflow-y-auto rounded-xl border border-border bg-panel p-4">
        ...
        <label className="mt-3 block text-[10.5px] font-semibold text-text-3">Journey layout<select value={settings.journeyLayout} onChange={(event) => setSettings({ ...settings, journeyLayout: event.target.value as 'horizontal' | 'vertical' })} className="mt-1 w-full rounded border border-border bg-panel-2 p-2 text-[11.5px] text-text"><option value="horizontal">Horizontal</option><option value="vertical">Vertical</option></select></label>
        <div className="mt-4 rounded-lg border border-border bg-panel-2 p-3"><div className="text-[11px] font-semibold text-text">Figma V2 · Planned</div><p className="mt-1 text-[10.5px] leading-relaxed text-text-3">Direct components, variables, Auto Layout and prototype connections. No authentication or placeholder action is enabled.</p></div>
        ...
        <button type="button" disabled={!!stage} onClick={() => void exportPackage()} className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg bg-accent py-2.5 text-[12px] font-semibold text-white disabled:opacity-50"><FileArchive size={13}/> Export Feature package</button>
        {stage && <div className="mt-3 flex items-center gap-2 text-[11px] text-accent-2"><Layers3 size={12} className="animate-pulse"/>{stage}</div>}
        {message && <p className="mt-3 break-words text-[10.5px] leading-relaxed text-text-2">{message}</p>}
        {warnings.length > 0 && <div className="mt-3 rounded-lg border border-warning/30 bg-warning/5 p-2.5 text-[10.5px] text-warning">{warnings.map((warning) => <div key={warning}>• {warning}</div>)}</div>}
        {history.length > 0 && <div className="mt-5"><div className="text-[10px] font-bold uppercase tracking-wide text-text-3">Export history</div>{history.slice(0, 5).map((item) => <div key={item.id} className="mt-2 border-t border-border pt-2 text-[10.5px] text-text-2"><div>{item.versionName} · {item.fileCount} files</div><div className="text-text-3">{new Date(item.createdAt).toLocaleString()}</div></div>)}</div>}
      </aside>
    </div>
  </div>
}

function Group({ title, children }: { title: string; children: React.ReactNode }) { return <div className="mb-5"><h2 className="mb-2 text-[10.5px] font-bold uppercase tracking-wide text-text-3">{title}</h2><div className="space-y-1.5">{children}</div></div> }
function Check({ checked, label, meta, onChange }: { checked: boolean; label: string; meta: string; onChange: () => void }) { return <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-border bg-panel px-3 py-2.5"><input type="checkbox" checked={checked} onChange={onChange}/><span className="min-w-0 flex-1"><span className="block text-[12px] font-medium capitalize text-text">{label}</span><span className="block truncate text-[10px] text-text-3">{meta}</span></span></label> }
function Option({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) { return <label className="mt-3 flex items-center gap-2 text-[11.5px] text-text-2"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)}/>{label}</label> }
