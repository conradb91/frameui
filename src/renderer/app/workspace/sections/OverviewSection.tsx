...
export function OverviewSection() {
  ...
  return <main className="min-w-0 flex-1 overflow-y-auto p-7">
    <div className="mx-auto max-w-5xl">
      <div className="flex items-start justify-between border-b border-border pb-5"><div><h1 className="text-[20px] font-semibold text-text">{activeProject.name}</h1><div className="mt-1 flex items-center gap-2 text-[10.5px] text-text-3"><span>{frameworkLabel(activeIndex?.framework)}</span>{model?.tokenSource !== 'none' && <><span>·</span><span>{styleLabel(model?.tokenSource)}</span></>}<span>·</span><span className="font-mono">{activeProject.path}</span></div></div><button type="button" onClick={() => void reindex()} disabled={indexing} className="flex items-center gap-1.5 rounded-[5px] border border-border bg-panel px-2.5 py-1.5 text-[10.5px] text-text-2 hover:text-text disabled:opacity-50"><RefreshCw size={12} className={indexing ? 'animate-spin' : ''} />{indexing ? 'Analysing…' : 'Reanalyse'}</button></div>

      {!model ? <AnalysisProgress /> : <>
        <section className="mt-6"><div className="mb-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-text-3">Product discovered</div><div className="grid grid-cols-4 border border-border bg-panel">{[
        ...
        ].map(([value, label]) => <button key={label} type="button" onClick={() => label === 'Needs review' || label === 'Render issues' ? setSection('review') : label === 'Screens' ? setSection('screens') : label === 'Components' ? setSection('components') : undefined} className="border-b border-r border-border px-4 py-3 text-left last:border-r-0 hover:bg-white/[0.025]"><span className="block text-[18px] font-semibold text-text">{value}</span><span className="mt-0.5 block text-[9.5px] text-text-3">{label}</span></button>)}</div></section>

        <div className="mt-7 grid grid-cols-[1.4fr_1fr] gap-7">
          <section><div className="mb-2 flex items-center justify-between"><span className="text-[10px] font-semibold uppercase tracking-[0.12em] text-text-3">Application areas</span><button type="button" onClick={() => setSection('screens')} className="text-[10px] text-blue-300">Open screens</button></div><div className="border-t border-border">{model.areas.map((area) => <div key={area.id} className="grid grid-cols-[1fr_80px] border-b border-border py-2 text-[10.5px]"><span className="text-text-2">{area.name}</span><span className="text-right font-mono text-text-3">{area.pageIds.length} screens</span></div>)}</div></section>
          <section><div className="mb-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-text-3">Analysis status</div><div className="border border-border bg-panel p-3"><StatusRow label="Project structure" value="Complete" ok /><StatusRow label="Routes" value={`${model.routes.length} found`} ok /><StatusRow label="Views" value={`${model.statistics.pages} found`} ok /><StatusRow label="Components" value={`${model.statistics.components} found`} ok /><StatusRow label="Relationships" value={`${model.statistics.connections} resolved`} ok={model.statistics.unresolvedRoutes === 0} /><StatusRow label="Review" value={model.statistics.issues ? `${model.statistics.issues} findings` : 'No findings'} ok={model.statistics.issues === 0} /></div></section>
        </div>
      </>}
    </div>
  </main>
...
function StatusRow({ label, value, ok }: { label: string; value: string; ok: boolean }) { return <div className="flex items-center justify-between border-b border-border py-2 last:border-0"><span className="flex items-center gap-2 text-[10px] text-text-2">{ok ? <CheckCircle2 size={11} className="text-green-400" /> : <AlertTriangle size={11} className="text-amber-400" />}{label}</span><span className="font-mono text-[9px] text-text-3">{value}</span></div> }
...
function styleLabel(value?: string): string { return value?.startsWith('tailwind') ? 'Tailwind CSS' : value === 'css-custom-properties' ? 'CSS variables' : '' }
