...
export function ComponentsSection() {
  ...
  useEffect(() => { setStructureCache({}) }, [activeIndex])

  useEffect(() => {
    let cancelled = false
    const visible = grouped.flatMap(([, items]) => items)
    const wanted = [...new Map((selected ? [selected, ...visible] : visible).map((component) => [component.source.filePath, component])).values()]
    const missing = wanted.filter((component) => structureCache[component.source.filePath] === undefined).slice(0, 60)
    if (!missing.length) return
    void Promise.all(missing.map(async (component) => [component.source.filePath, await window.frameui.project.getPageStructure(component.source.filePath).catch(() => [])] as const))
      .then((entries) => { if (!cancelled) setStructureCache((previous) => ({ ...previous, ...Object.fromEntries(entries) })) })
    return () => { cancelled = true }
  }, [activeIndex, grouped, selected, structureCache])
  ...
  return <div className="flex min-h-0 flex-1">
    <aside className="flex w-[260px] shrink-0 flex-col border-r border-border bg-bg-raised"><div className="flex h-10 items-center justify-between px-3"><span className="text-[12px] font-semibold tracking-normal text-text-3">Components</span><span className="font-mono text-[12px] text-text-3">{components.length}</span></div><div className="border-b border-border px-2 pb-2"><div className="flex h-7 items-center gap-2 rounded-[5px] border border-border bg-panel px-2"><Search size={12} className="text-text-3" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search components…" className="min-w-0 flex-1 bg-transparent text-[12px] text-text outline-none placeholder:text-text-3" /></div></div><div className="flex-1 overflow-y-auto px-1.5 py-2">{grouped.map(([group, items]) => <div key={group} className="mb-3"><div className="mb-1 flex justify-between px-1.5 text-[12px] font-semibold tracking-wide text-text-3"><span>{group}</span><span>{items.length}</span></div>{items.map((component) => <button key={component.id} type="button" onClick={() => setSelectedId(component.id)} className={`flex w-full items-center gap-2 rounded-[4px] px-2 py-1.5 text-left ${selected?.id === component.id ? 'bg-selected text-text' : 'text-text-2 hover:bg-hover'}`}><ComponentThumbnail component={component} structure={structureCache[component.source.filePath] ?? null} size="sm" /><span className="truncate text-[12px]">{component.name}</span></button>)}</div>)}</div></aside>
    <main className="min-w-0 flex-1 overflow-y-auto p-4">{selected ? <><div className="mb-5 flex items-baseline gap-3"><h1 className="text-[13px] font-semibold text-text">{selected.name}</h1><span className="text-[12px] text-text-3">{usedIn.length} screen{usedIn.length === 1 ? '' : 's'}</span></div><div className="grid grid-cols-2 gap-4"><ComponentState label="Default" structure={structure} /><ComponentState label="Hover" structure={structure} muted /><ComponentState label="Active" structure={structure} muted /><ComponentState label="Disabled" structure={structure} muted /></div>{usedIn.length > 0 && <section className="mt-3"><div className="mb-2 text-[12px] font-semibold tracking-wide text-text-3">Used in</div><div className="border-t border-border">{usedIn.map((page) => <button key={page.id} type="button" onClick={() => openUsage(page.id)} className="grid w-full grid-cols-[1fr_1fr_90px] border-b border-border py-2 text-left text-[12px] hover:bg-hover"><span className="text-text-2">{page.name}</span><span className="font-mono text-[12px] text-text-3">{page.route}</span><span className="text-right text-accent-2">Open screen</span></button>)}</div></section>}</> : <div className="flex h-full items-center justify-center text-[12px] text-text-3">No reusable components detected.</div>}</main>
    <aside className="w-[280px] shrink-0 border-l border-border bg-bg-raised">{selected ? <><div className="border-b border-border p-3"><div className="text-[12px] font-semibold text-text">{selected.name}</div><div className="mt-0.5 text-[12px] text-text-3">Component inspector</div></div><div className="p-3"><InspectorField label="Source" value={selected.source.filePath} mono /><InspectorField label="Source type" value={selected.exportKind === 'template' ? 'Template component' : `${selected.exportKind} export`} /><InspectorField label="Usage" value={`${usedIn.length} screens`} /><InspectorField label="Elements" value={String(countNodes(structure))} /><div className="mt-4 rounded-[4px] border border-border bg-panel p-2.5 text-[12px] leading-relaxed text-text-3">Runtime props, computed styles, variants and accessibility become available after running the application.</div></div></> : null}</aside>
  </div>
}

function ComponentState({ label, structure, muted }: { label: string; structure: PageStructureItem[]; muted?: boolean }) { return <div><div className="mb-1.5 text-[12px] text-text-3">{label}</div><div className={`aspect-[16/8] overflow-hidden rounded-[5px] border border-border bg-panel ${muted ? 'opacity-65' : ''}`}><StructurePreview structure={structure} compact /></div></div> }
function InspectorField({ label, value, mono }: { label: string; value: string; mono?: boolean }) { return <div className="border-b border-border py-2"><div className="mb-1 text-[12px] text-text-3">{label}</div><div className={`break-all text-[12px] text-text-2 ${mono ? 'font-mono text-[12px]' : ''}`}>{value}</div></div> }
