...
export function CapturesSection() {
  ...
  return <div className="flex min-h-0 flex-1">
    <aside className="flex w-[260px] shrink-0 flex-col border-r border-border bg-bg-raised">
      <div className="flex h-10 items-center justify-between px-3"><span className="text-[12px] font-semibold tracking-normal text-text-3">Captures</span><span className="font-mono text-[12px] text-text-3">{sorted.length}</span></div>
      <div className="flex-1 overflow-y-auto px-1.5 py-2">{sorted.map((capture) => <button key={capture.id} type="button" onClick={() => setSelectedId(capture.id)} className={`block w-full rounded-[4px] px-2 py-2 text-left ${selected?.id === capture.id ? 'bg-selected text-text' : 'text-text-2 hover:bg-hover'}`}><span className="block truncate text-[12px]">{capture.url}</span><span className="mt-0.5 block text-[12px] text-text-3">{formatRelativeTime(capture.capturedAt)}</span></button>)}{sorted.length === 0 && <div className="px-2 py-3 text-[12px] leading-relaxed text-text-3">No captures yet — open Capture Session and click Capture This Page.</div>}</div>
    </aside>
    <main className="min-w-0 flex-1 overflow-y-auto p-4">{selected ? <>
      <div className="mb-5">{screenshotUrl ? <img src={screenshotUrl} alt="Captured page screenshot" className="max-h-[360px] rounded-[5px] border border-border" /> : <div className="flex h-[140px] items-center justify-center rounded-[5px] border border-border bg-panel text-[12px] text-text-3">No screenshot</div>}</div>
      <div className="mb-2 text-[12px] font-semibold tracking-wide text-text-3">Structure</div>
      <div className="overflow-x-auto rounded-[5px] border border-border bg-panel p-2 font-mono text-[12px] leading-relaxed"><ElementRow element={selected.root} depth={0} selectedElement={selectedElement} onSelect={setSelectedElement} /></div>
    </> : <div className="flex h-full items-center justify-center text-center text-[12px] text-text-3">No captures yet — open Capture Session and click Capture This Page.</div>}</main>
    <aside className="w-[280px] shrink-0 border-l border-border bg-bg-raised">{selected ? <>
      <div className="border-b border-border p-3"><div className="truncate text-[12px] font-semibold text-text" title={selected.url}>{selected.url}</div><div className="mt-0.5 text-[12px] text-text-3">Capture inspector</div></div>
      <div className="p-3">
        ...
        {selectedElement ? <>
          <div className="mb-1 mt-4 text-[12px] font-semibold tracking-wide text-text-3">Selected element — {elementLabel(selectedElement)}</div>
          {Object.keys(selectedElement.styles).length === 0 && !selectedElement.aria && <div className="py-2 text-[12px] text-text-3">No styles or ARIA attributes recorded.</div>}
        ...
        </> : <div className="mt-4 text-[12px] leading-relaxed text-text-3">Click an element in the structure to inspect its computed styles and ARIA attributes.</div>}
      </div>
    </> : null}</aside>
  </div>
}
...
function ElementRow({ element, depth, selectedElement, onSelect }: { element: CapturedElement; depth: number; selectedElement: CapturedElement | null; onSelect: (element: CapturedElement) => void }) {
  return <div>
    <button type="button" onClick={() => onSelect(element)} style={{ paddingLeft: `${depth * 14}px` }} className={`block w-full truncate rounded-[3px] py-0.5 pr-1 text-left ${selectedElement === element ? 'bg-selected text-text' : 'text-text-2 hover:bg-hover'}`}>{elementLabel(element)} <span className="text-text-3">{Math.round(element.rect.width)}×{Math.round(element.rect.height)}</span></button>
...
function InspectorField({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return <div className="border-b border-border py-2"><div className="mb-1 text-[12px] text-text-3">{label}</div><div className={`break-all text-[12px] text-text-2 ${mono ? 'font-mono text-[12px]' : ''}`}>{value}</div></div>
}
