...
export const useDesignStore = create<DesignState>((set, get) => ({
  ...
  loadDesignState: async (projectId, designStateId, alternativeId = null) => {
    ...
    const existing = await window.frameui.workspace.getDesignTree(projectId, ownerId)
    const tree = existing?.tree ?? createDefaultTree(crypto.randomUUID())
...
function scheduleSave(get: () => DesignState, set: (partial: Partial<DesignState>) => void) {
  ...
  const timer = setTimeout(async () => {
    const { projectId, flowId, screenId, designStateId, alternativeId, tree } = get()
    if (!projectId || !tree) return
    set({ saving: true })
    if (designStateId) {
      await window.frameui.workspace.saveDesignTree({
        ownerId: alternativeId ?? designStateId,
        projectId,
        tree,
