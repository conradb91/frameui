...
export function FeatureReviewPanel(props: Props) {
  ...
  return <div className="flex h-full flex-col">
    <div className="border-b border-border p-2.5">
      <div className="mb-2 flex gap-1">{(['open', 'resolved', 'all'] as const).map((value) => <button key={value} onClick={() => setTab(value)} className={`rounded px-2 py-1 text-[11px] font-semibold capitalize ${tab === value ? 'bg-selected text-accent-2' : 'text-text-3'}`}>{value}</button>)}</div>
      <div className="mb-2 flex gap-1">
        <select value={viewport} onChange={(e) => setViewport(e.target.value as typeof viewport)} className="min-w-0 flex-1 rounded border border-border bg-panel-2 px-1 py-1 text-[12px] text-text"><option value="all">All viewports</option><option value="desktop">Desktop</option><option value="tablet">Tablet</option><option value="mobile">Mobile</option></select>
        <select value={context} onChange={(e) => setContext(e.target.value as typeof context)} className="min-w-0 flex-1 rounded border border-border bg-panel-2 px-1 py-1 text-[12px] text-text"><option value="all">Current + Proposed</option><option value="current">Current</option><option value="proposed">Proposed</option></select>
      </div>
      <div className="mb-2 flex gap-1"><select value={pageFilter} onChange={(e) => setPageFilter(e.target.value)} className="min-w-0 flex-1 rounded border border-border bg-panel-2 px-1 py-1 text-[12px] text-text"><option value="all">All pages</option>{pages.map((value) => <option key={value} value={value}>{value}</option>)}</select><select value={stateFilter} onChange={(e) => setStateFilter(e.target.value)} className="min-w-0 flex-1 rounded border border-border bg-panel-2 px-1 py-1 text-[12px] text-text"><option value="all">All states</option>{states.map((value) => <option key={value} value={value}>{value}</option>)}</select></div>
      <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={3} placeholder={props.selectedNode ? `Annotate selected ${props.selectedNode.kind}…` : 'Annotate this page…'} className="w-full resize-none rounded border border-border bg-panel-2 p-2 text-[12px] text-text outline-none" />
      <div className="mt-1.5 flex gap-1.5"><select value={newContext} onChange={(e) => setNewContext(e.target.value as Annotation['context'])} className="min-w-0 flex-1 rounded border border-border bg-panel-2 px-1 text-[12px] text-text"><option value="proposed">Proposed</option><option value="current">Current page</option></select><select value={priority} onChange={(e) => setPriority(e.target.value as Annotation['priority'])} className="min-w-0 flex-1 rounded border border-border bg-panel-2 px-1 text-[12px] text-text"><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option></select><button disabled={!comment.trim() || !props.pageRef} onClick={() => void create()} className="flex items-center gap-1 rounded bg-accent px-2 py-1 text-[11px] font-semibold text-on-accent disabled:opacity-40"><Plus size={11}/>Add</button></div>
    </div>
    <div className="flex-1 overflow-y-auto p-2">{visible.map((item) => <div key={item.id} className="mb-2 rounded-md border border-border bg-panel-2 p-2">
      <button onClick={() => props.onJump(item)} className="w-full text-left"><div className="flex items-center gap-1 text-[11px] text-text-3">{item.needsAttention ? <AlertTriangle size={10} className="text-warning"/> : <MapPin size={10} className="text-accent-2"/>}{item.viewport} · {item.priority} · {item.status}</div><div className="mt-1 text-[11px] leading-relaxed text-text">{item.comment}</div><div className="mt-1 truncate font-mono text-[11px] text-text-3">{item.elementLabel ?? item.pageRef.pageId}</div></button>
      <div className="mt-2 flex gap-2">{item.status === 'resolved' ? <button onClick={() => void setStatus(item, 'reopened')} className="flex items-center gap-1 text-[11px] text-text-3"><CornerUpLeft size={10}/>Reopen</button> : <button onClick={() => void setStatus(item, 'resolved')} className="flex items-center gap-1 text-[11px] text-success"><Check size={10}/>Resolve</button>}<button onClick={() => void edit(item)} className="ml-auto text-text-3 hover:text-text"><Pencil size={10}/></button><button onClick={() => void remove(item)} className="text-text-3 hover:text-danger"><Trash2 size={10}/></button></div>
    </div>)}{visible.length === 0 && <div className="p-3 text-center text-[11px] text-text-3">No review items in this view.</div>}</div>
  </div>
}
