  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-10 items-center justify-between px-3">
        <span className="text-[11px] font-semibold tracking-normal text-text-3">Journeys</span>
        <button type="button" onClick={() => setCreating(true)} className="rounded p-1 text-text-3 hover:bg-hover hover:text-text" title="New Journey"><Plus size={14} /></button>
      </div>
      {creating && (
        <div className="mx-2 mb-2 flex gap-1">
          <input autoFocus value={name} onChange={(event) => setName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void create(); if (event.key === 'Escape') setCreating(false) }} placeholder="Journey name" className="min-w-0 flex-1 rounded border border-border bg-panel px-2 py-1.5 text-[12px] text-text outline-none focus:border-accent" />
          <button type="button" onClick={() => void create()} className="rounded bg-accent px-2 text-on-accent"><Check size={12} /></button>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-1.5 pb-2">
        ...
        {journeys.map((journey) => (
          <div key={journey.id} className={`group mb-1 rounded-md ${selectedJourneyId === journey.id ? 'bg-selected' : 'hover:bg-hover'}`}>
            {renamingId === journey.id ? (
              <div className="flex gap-1 p-1.5"><input autoFocus value={renameValue} onChange={(event) => setRenameValue(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void finishRename(journey.id); if (event.key === 'Escape') setRenamingId(null) }} className="min-w-0 flex-1 rounded border border-border bg-panel px-1.5 text-[12px] text-text outline-none" /><button type="button" onClick={() => void finishRename(journey.id)}><Check size={12} /></button></div>
            ) : (
              <div className="flex items-center">
                <button type="button" onClick={() => void select(journey.id)} className="min-w-0 flex-1 px-2 py-2 text-left">
                  ...
                  <div className="text-[11px] text-text-3">{journey.steps.length} steps · {journey.connections.length} interactions</div>
                </button>
                ...
                <button type="button" onClick={() => void remove(journey.id)} className="hidden p-1.5 text-text-3 hover:text-danger group-hover:block"><Trash2 size={11} /></button>
              </div>
            )}
          </div>
        ))}
