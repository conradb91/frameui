...
export function ProjectAssetPreview({ structure, visuals, name }: { structure: PageStructureItem[]; visuals: ProjectVisuals; name: string }) {
  ...
  return <div className="relative h-16 w-full overflow-hidden rounded border border-border bg-panel"><iframe title={`${name} preview`} tabIndex={-1} sandbox="" srcDoc={html} className="pointer-events-none absolute left-0 top-0 h-[256px] w-[800px] origin-top-left scale-[.25] border-0"/></div>
}
