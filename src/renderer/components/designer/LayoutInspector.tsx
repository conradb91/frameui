...
function PlaceholderFields({
  ...
  return (
    <>
      ...
      {intelligence && intelligence.props.length > 0 && <Field label="Detected component properties">{intelligence.props.map((prop) => <div key={prop.name} className="mb-2 text-xs"><span className="text-text">{prop.name}</span><p className="mt-1 text-text-3">{prop.values.join(' · ') || prop.type}</p></div>)}<p className="text-[10px] text-text-3">Application-dependent properties are shown as source information.</p></Field>}
