...
export function JourneyInspector({ projectId, featureId: _featureId, journeyId }: { projectId: string; featureId: string; journeyId: string | null }) {
  ...
  if (step) return (
    <div className="flex flex-col gap-3">
      ...
      <div className="text-[11px] leading-relaxed text-text-3">Double-click the card to open this page/state in Design. Reference-only steps remain read-only.</div>
    </div>
  )
  ...
  if (connection) return (
    <div className="flex flex-col gap-3">
      ...
      <Field label="Destination"><div className="truncate font-mono text-[11px] text-text-2">{journey.steps.find((item) => item.id === connection.toStepId)?.pageRef.pageId ?? connection.toStepId}</div></Field>
      ...
      <div className="text-[11px] text-text-3">Delete: select this connection on the canvas and press Backspace.</div>
    </div>
  )
...
function InspectorHeading({ children }: { children: string }) { return <div className="text-[11px] font-semibold tracking-normal text-text-3">{children}</div> }
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <div><div className="mb-1 text-[11px] text-text-3">{label}</div>{children}</div> }
