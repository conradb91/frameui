...
export function ScreenNode({ data, selected }: NodeProps<ScreenFlowNode>) {
  ...
  return (
    <div
      className={`w-[240px] overflow-hidden rounded-[6px] border bg-panel-2 shadow-sm ${
        referenceOnly ? 'opacity-70' : ''
      } ${selected ? 'border-accent-2 shadow-none' : 'border-border'}`}
    >
      ...
      <div className="relative h-32 overflow-hidden bg-panel">
        {screen ? <StructurePreview structure={screen.structure} compact /> : <div className="flex h-full items-center justify-center bg-panel text-[11px] text-text-3">Blank screen</div>}
        {referenceOnly && (
          <span className="absolute right-1.5 top-1.5 rounded-md border border-border bg-panel px-1.5 py-0.5 text-[11px] font-semibold tracking-wide text-text-3">
            Reference
          </span>
        )}
      </div>
      <div className="border-t border-border px-3 py-2.5">
        ...
        <div className="mt-0.5 truncate font-mono text-[11px] text-text-3">{screen?.route ?? (data.source.type === 'blank' ? 'Blank screen' : data.source.pageFilePath)}</div>
