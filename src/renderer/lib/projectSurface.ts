...
export function renderProjectNode(node: DesignNode, visuals: ProjectVisuals, breakpoint: Breakpoint): string {
  ...
  if (node.kind === 'placeholder') {
    ...
    Object.assign(style, { display: 'flex', flexDirection: override?.direction ?? node.direction, gap: override?.gap ?? node.gap, alignItems: node.align === 'start' ? 'flex-start' : node.align, flexWrap: node.wrap ? 'wrap' : 'nowrap' })
  } else if (node.kind === 'grid') Object.assign(style, { display: 'grid', gridTemplateColumns: `repeat(${override?.columns ?? node.columns}, minmax(0, 1fr))`, columnGap: node.columnGap, rowGap: node.rowGap })
...
export function projectDocument(node: DesignNode, visuals: ProjectVisuals, breakpoint: Breakpoint, maxWidth?: number): string {
  return `<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data:; font-src data:; style-src 'unsafe-inline'; form-action 'none'; base-uri 'none'"><style>${visuals.css.replace(/<\/style/gi, '<\\/style')}\nhtml,body{margin:0;min-height:100%;}#frameui-surface{max-width:${maxWidth ? `${maxWidth}px` : 'none'};margin-inline:auto;min-height:100vh} [data-frameui-selected]{outline:2px solid #477d72!important;outline-offset:1px}</style></head><body><div id="frameui-surface">${renderProjectNode(node, visuals, breakpoint)}</div></body></html>`
}
